...
local hpp = Tunnel.getInterface("hpp_police")

hppC = {}
Tunnel.bindInterface("hpp_police", hppC)
...
customs = {
    ...
    {title = "Parachoque", mod = 1, startingcost = 1000, data = {

    }},
    {title = "Parachoque Traseiro", mod = 2, startingcost = 1000, data = {

    }},
    {title = "Saias", mod = 3, startingcost = 500, data = {

    }},
    {title = "Escapamento", mod = 4, startingcost = 1000, data = {

    }},
    {title = "Santantônio", mod = 5, startingcost = 5000, data = {

    }},
    {title = "Exaustor", mod = 6, startingcost = 1000, data = {

    }},
    {title = "Capô", mod = 7, startingcost = 2000, data = {

    }},
    {title = "Pára-choque esquerdo", mod = 8, startingcost = 1000, data = {

    }},
    {title = "Pára-choque direito", mod = 9, startingcost = 1000, data = {

    }},
    {title = "Teto", mod = 10, startingcost = 1500, data = {
...
AddEventHandler("customs:receive",function(title, data, value)
    if title == "Spoiler" or title == "Parachoque" or title == "Parachoque Traseiro" or title == "Saias" or title == "Escapamento" or title == "Santantônio" or title == "Exaustor" or title == "Capô" or title == "Pára-choque esquerdo" or title == "Pára-choque direito" or title == "Teto" or title == "Suspension" or title == "Transmission" or title == "Engine" or title == "Brakes" or title == "Armour" or title == "Windows" or title == "Horn" or title == "Plate" or title == "Turbo" then
...
function openCustoms()
	...
	for i = 1,11 do
        ...
        SetVehicleModKit(currentvehicle, 0)
         if GetNumVehicleMods(currentvehicle, tonumber(customs[i].mod)) ~= 0 then
            --print(i)
            --print(customs[i].mod)
		 	local mt = tonumber(customs[i].mod)
					...
					if label ~= nil then
						...
						if prevmodname == nil and prevmodname ~= modname then
							if modname ~= "NULL" then
                                ...
                                startingcost = startingcost + 500
								table.insert(customs[i].data,{name=label,cost=startingcost,mt=mt,mod=a})
                                prevmodname = modname
					 		end
					 	end
			 		end
    ...
    Vehicledata = {
        ...
        ["Parachoque"]=GetVehicleMod(currentvehicle, 1), -- Vehicle Mod 1
        ["Parachoque Traseiro"]=GetVehicleMod(currentvehicle, 2), -- Vehicle Mod 2
        ["Saias"]=GetVehicleMod(currentvehicle, 3), -- Vehicle Mod 3
        ["Escapamento"]=GetVehicleMod(currentvehicle, 4), -- Vehicle Mod 4
        ["Santantônio"]=GetVehicleMod(currentvehicle, 5), -- Vehicle Mod 5
        ["Exaustor"]=GetVehicleMod(currentvehicle, 6), -- Vehicle Mod 6
        ["Capô"]=GetVehicleMod(currentvehicle, 7), -- Vehicle Mod 7
        ["Pára-choque esquerdo"]=GetVehicleMod(currentvehicle, 8), -- Vehicle Mod 8
        ["Pára-choque direito"]=GetVehicleMod(currentvehicle, 9), -- Vehicle Mod 9
        ["Teto"]=GetVehicleMod(currentvehicle, 10), -- Vehicle Mod 10
    ...
    if IsEntityAVehicle(veh) then
		...
		if vname then
			...
			if DoesEntityExist(veh) then
                ...
                wheel = custom.wheel
                print(vname,custom,plate)
                TriggerServerEvent("finalized:custom",custom,plate)
				return vname,custom
			end
		end
	end
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
emplacement_customs = { 
    {locked = false,name="Oficina", colour=3, id=72, locked = false, outside = { x = 928.33892822266, y = -971.30456542969, z = 38.999816894531, heading = -90.0}, inside = {x = 937.57763671875, y = -970.85815429688, z = 38.993048858643, heading = -90.0}},
}
    ...
    while true do
        ...
        for k,v in ipairs(emplacement_customs) do
            if(Vdist(pos.x, pos.y, pos.z, v.outside.x, v.outside.y, v.outside.z) < 15.0)then
                idle = 1
                DrawMarker(23, v.outside.x, v.outside.y, v.outside.z - 0.5, 0, 0, 0, 0, 0, 0, 1.5001, 1.5001, 0.5001, 177, 0, 0,40, 0, 0, 0,0)
                if(Vdist(pos.x, pos.y, pos.z, v.outside.x, v.outside.y, v.outside.z) < 5.0)then
                    ...
                    if v.locked == false then
                        drawCustomsTxt("Pressione ~b~ENTER~w~ para entrar na ~b~Oficina ",4,1,0.5,0.8,1.0,255,255,255,255)
                    --incircle = true
                        local v = GetVehiclePedIsIn(GetPlayerPed(-1), false)
                        if IsControlJustReleased(1, 201) then -- INPUT_CELLPHONE_DOWN
                            if hpp.checkPerm("mechPerm") then
                                ...
                                --notify("<b>~r~[AVISO] ~w~Você não é um mecânico!</b>")
                                TriggerEvent("Notify", "negado", "Você não é um mecânico!")
                            end
                        end
                    else
...
function UpdateCar()
    ...
    SetVehicleMod(currentvehicle, 1, tonumber(Vehicledata["Parachoque"]))
    SetVehicleMod(currentvehicle, 2, tonumber(Vehicledata["Parachoque Traseiro"]))
    SetVehicleMod(currentvehicle, 3, tonumber(Vehicledata["Saias"]))
    SetVehicleMod(currentvehicle, 4, tonumber(Vehicledata["Escapamento"]))
    SetVehicleMod(currentvehicle, 5, tonumber(Vehicledata["Santantônio"]))
    SetVehicleMod(currentvehicle, 6, tonumber(Vehicledata["Exaustor"]))
    SetVehicleMod(currentvehicle, 7, tonumber(Vehicledata["Capô"]))
    SetVehicleMod(currentvehicle, 8, tonumber(Vehicledata["Pára-choque esquerdo"]))
    SetVehicleMod(currentvehicle, 9, tonumber(Vehicledata["Pára-choque direito"]))
    SetVehicleMod(currentvehicle, 10, tonumber(Vehicledata["Teto"]))
