...
RegisterNetEvent("hpp:ToggleDebug")
AddEventHandler("hpp:ToggleDebug",function()
...
function GetObject()
    ...
    repeat
        ...
        if distance < 10.0 then
            ...
            --FreezeEntityPosition(ped, inFreeze)
	    	if IsEntityTouchingEntity(GetPlayerPed(-1), ped) then
	    		DrawText3Ds(pos["x"],pos["y"],pos["z"]+1, "Objeto: " .. ped .. " Hash: " .. GetEntityModel(ped) .. " IN CONTACT" )
	    	else
	    		DrawText3Ds(pos["x"],pos["y"],pos["z"]+1, "Objeto: " .. ped .. " Hash: " .. GetEntityModel(ped) .. "" )
	    	end
...
function getNPC()
    ...
    repeat
        ...
        if canPedBeUsed(ped) and distance < 30.0 and (distanceFrom == nil or distance < distanceFrom) then
	    	...
	    	if IsEntityTouchingEntity(GetPlayerPed(-1), ped) then
	    		DrawText3Ds(pos["x"],pos["y"],pos["z"], "Ped: " .. ped .. " Modelo: " .. GetEntityModel(ped) .. " Relationship HASH: " .. GetPedRelationshipGroupHash(ped) .. " IN CONTACT" )
	    	else
	    		DrawText3Ds(pos["x"],pos["y"],pos["z"], "Ped: " .. ped .. " Modelo: " .. GetEntityModel(ped) .. " Relationship HASH: " .. GetPedRelationshipGroupHash(ped) )
	    	end
...
Citizen.CreateThread( function()
    ...
    while true do 
        ...
        if dickheaddebug then
            ...
            --drawTxtS(0.8, 0.48, 0.4,0.4,0.30, "IP: " .. ip, 55, 155, 55, 255)
            drawTxtS(0.8, 0.50, 0.4,0.4,0.30, "Rotação: " .. GetEntityHeading(GetPlayerPed(-1)), 55, 155, 55, 255)
            drawTxtS(0.8, 0.52, 0.4,0.4,0.30, "Cordenada: " .. pos, 55, 155, 55, 255)
            drawTxtS(0.8, 0.54, 0.4,0.4,0.30, "Entidade em anexo: " .. GetEntityAttachedTo(GetPlayerPed(-1)), 55, 155, 55, 255)
            drawTxtS(0.8, 0.56, 0.4,0.4,0.30, "Vida: " .. GetEntityHealth(GetPlayerPed(-1)), 55, 155, 55, 255)
            drawTxtS(0.8, 0.58, 0.4,0.4,0.30, "H a G: " .. GetEntityHeightAboveGround(GetPlayerPed(-1)), 55, 155, 55, 255)
            drawTxtS(0.8, 0.60, 0.4,0.4,0.30, "Modelo: " .. GetEntityModel(GetPlayerPed(-1)), 55, 155, 55, 255)
            drawTxtS(0.8, 0.62, 0.4,0.4,0.30, "Velocidade: " .. GetEntitySpeed(GetPlayerPed(-1)), 55, 155, 55, 255)
            drawTxtS(0.8, 0.64, 0.4,0.4,0.30, "Frame Time: " .. GetFrameTime(), 55, 155, 55, 255)
            drawTxtS(0.8, 0.66, 0.4,0.4,0.30, "Rua: " .. currentStreetName, 55, 155, 55, 255)
            ...
            if IsControlJustReleased(0, 38) then
                if inFreeze then
                    ...
                    tvRP.notify("Freeze Disabled")
                else
                    inFreeze = true             
                    --TriggerEvent("DoShortHudText",'Freeze Enabled',3)
                    tvRP.notify("Freeze Enabled")
                end
            end
            ...
            if IsControlJustReleased(0, 47) then
                if lowGrav then
                    ...
                    tvRP.notify("Low Grav Disabled")
                else
                    lowGrav = true              
                    --TriggerEvent("DoShortHudText",'Low Grav Enabled',3)
                    tvRP.notify("Low Grav Enabled")
                end
            end
