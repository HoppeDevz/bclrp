...
TokoVoipConfig = {
	...
	plugin_data = {
		...
		TSChannel = "In Game",
		TSPassword = "00x0898312312ghhh", -- TeamSpeak channel password (can be empty)
		...
		TSServer = "bclrp.serverfg.top", -- TeamSpeak server address to be displayed on blocking screen
		...
		localNamePrefix = "[BCLRP] ", -- If set, this prefix will be added to the user's teamspeak display name
	}
};
...
AddEventHandler("onClientResourceStart", function(resource)
	if (resource == GetCurrentResourceName()) then	--	Initialize the script when this resource is started
		Citizen.CreateThread(function()
			TokoVoipConfig.plugin_data.localName = "Cidadão " .. GetPlayerServerId(PlayerId()); -- Set the local name
		end);
